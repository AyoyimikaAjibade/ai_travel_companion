...
def call_gemini(user_message: str, current_slots: Optional[Slots]) -> dict:    
    ...
    prompt = (
        ...
        "FILLING RULES:\n"
            ...
            "     - If hotel.request is null but hotel.amenities already has any values in Current Slots (e.g., \"breakfast\" prefilled by the app) OR the user clicks/selects any suggested-reply option that is an amenity (e.g., \"Breakfast only\"), treat that as a positive hotel request and set hotel.request = True.\n"
            ...
            "9) **reply**:\n"
            "   • If there is any item in the `missing`, fill reply field to request filling the missing field in **everyday language ONE BY ONE. DO NOT MENTION ALL MISSING FIELDS AT ONCE.**.\n"
            "   • Do NOT use slot field names or IATA codes. MUST use everyday language\n"
            "   • Again! ask missing field ONE BY ONE!\n"
